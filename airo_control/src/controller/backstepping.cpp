...
mavros_msgs::AttitudeTarget BACKSTEPPING::solve(const geometry_msgs::PoseStamped& current_pose, const geometry_msgs::TwistStamped& current_twist, const geometry_msgs::AccelStamped& current_accel, const airo_message::Reference& ref, const sensor_msgs::BatteryState& battery_state){  
    ...
    a_z = (e_z1+g+ref.ref_accel.linear.z-param.k_z1*(e_z2 + param.k_z1*e_z1)-param.k_z2*e_z2)/(cos(current_euler.x())*cos(current_euler.y()));
    ...
    u_x = (e_x1+ref.ref_accel.linear.x-param.k_x1*(e_x2+param.k_x1*e_x1)-param.k_x2*e_x2)/a_z;
    ...
    u_y = (e_y1+ref.ref_accel.linear.y-param.k_y1*(e_y2+param.k_y1*e_y1)-param.k_y2*e_y2)/a_z;
