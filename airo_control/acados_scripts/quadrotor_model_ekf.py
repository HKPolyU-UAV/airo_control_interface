...
def export_quadrotor_model() -> AcadosModel:
    ...
    dy = v
    dz = w
    du = (cos(phi)*sin(theta)*cos(psi) + sin(phi)*sin(psi)) * thrust/hover_thrust*g + disturbance_x
    dv = (cos(phi)*sin(theta)*sin(psi) - sin(phi)*cos(psi)) * thrust/hover_thrust*g + disturbance_y
    dw = -g + cos(theta) * cos(phi) * thrust/hover_thrust*g + disturbance_z
