...
def main():
    ...
    N = 40      # Prediction steps --> time steps: 0.025
    ocp.dims.N = N

    # set parameters
    adaptive_param = np.zeros(nparam)  # Initialize adaptive parameters
    ocp.parameter_values = adaptive_param    

    # set cost
    W_x = np.diag([200, 200, 200, 30, 30, 30, 10, 10])    #Q_mat x y z u v w phi theta
    W_u = np.diag([5000, 2000, 2000])                     #R_mat Thrust phi_cmd theta_cmd
    W = block_diag(W_x, W_u)
    ocp.cost.W_e = W_x
    ocp.cost.W = W
    ...
    # set constraints
    u_min = np.array([0, -math.pi/2, -math.pi/2]) # 0 -45deg -45deg (0 or negative for min.)
    u_max = np.array([1, math.pi/2, math.pi/2])   # 1 45deg 45deg (positive for max.)
    x_min = np.array([-math.pi/2,-math.pi/2])     # -45deg -45 deg 
    x_max = np.array([math.pi/2,math.pi/2])       # 45deg 45deg
    ...
    # Create EKF variables
    ekf_x = np.zeros(nx)  # Initial state estimate
    ekf_P = 1e-6 * np.eye(nx)  # Initial state covariance matrix    

    # EKF process noise covariance matrix
    ekf_Q = np.diag([1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2, 1e-2])

    # EKF measurement noise covariance matrix
    ekf_R = np.diag([1e-1, 1e-1, 1e-1])
    ...
    def ekf_update(x, P, z):
        ...
        x_pred = model.f_impl_expr(x, np.zeros(nu), adaptive_param)

        # Predicted covariance estimate
        P_pred = A @ P @ A.T + B @ ekf_Q @ B.T

        # Measurement residual
        y = z - C @ x_pred

        # Kalman gain
        K = P_pred @ C.T @ np.linalg.inv(C @ P_pred @ C.T + ekf_R)
    ...
    for k in range(N):
        ...
        ocp.cost.yref[:nx] = x_ref

        # Measure the quadrotor pose
        z = measure_pose()

        # Perform EKF update
        ekf_x, ekf_P = ekf_update(ekf_x, ekf_P, z)

        # Set initial state estimate for the OCP
        ocp.constraints.x0 = ekf_x

        # Solve the OCP
        ocp_solver.solve()

        # Get optimal control input
        u_opt = ocp_solver.get(ocp_solver.STATES, 1)

        # Apply control input to the quadrotor
        apply_control_input(u_opt)

        # Measure the quadrotor pose again
        z = measure_pose()

        # Perform EKF update again
        ekf_x, ekf_P = ekf_update(ekf_x, ekf_P, z)

        # Update reference trajectory for the next iteration
        x_ref = update_reference_trajectory() # Implement your own function for updating the reference trajectory
