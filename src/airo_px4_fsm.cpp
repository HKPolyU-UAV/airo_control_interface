...
AIRO_PX4_FSM::AIRO_PX4_FSM(ros::NodeHandle& nh){
    ...
    nh.getParam("airo_px4_node/pitch_channel",rc_param.PITCH_CHANNEL);
    nh.getParam("airo_px4_node/roll_channel",rc_param.ROLL_CHANNEL);
    nh.getParam("airo_px4_node/fsm_channel",rc_param.FSM_CHANNEL);
    nh.getParam("airo_px4_node/command_channel",rc_param.COMMAND_CHANNEL);
    nh.getParam("airo_px4_node/reboot_channel",rc_param.REBOOT_CHANNEL);
    nh.getParam("airo_px4_node/reverse_throttle",rc_param.REVERSE_THROTTLE);
    nh.getParam("airo_px4_node/reverse_yaw",rc_param.REVERSE_YAW);
    nh.getParam("airo_px4_node/reverse_pitch",rc_param.REVERSE_PITCH);
        ...
        case AUTO_HOVER:{
            ...
            // To AUTO_LAND
                ...
                if (external_command_received(current_time)){
                    ROS_WARN("[AIRo PX4] Reject AUTO_LAND mode. Stop sending external commands and try again!");
                    set_ref_with_rc();
            ...
            // To POS_COMMAND
                ...
                if (external_command.ref_pose.size() != QUADROTOR_N + 1 || external_command.ref_twist.size() != QUADROTOR_N + 1){
                    ROS_ERROR_STREAM_THROTTLE(1.0,"[AIRo PX4] Reject POS_COMMAND. External command size is wrong!");
                    break;
                }
...
bool AIRO_PX4_FSM::toggle_offboard(bool flag){
	...
	if (flag){
		...
		if (!(setmode_srv.call(offboard_setmode) && offboard_setmode.response.mode_sent)){
            ...
            ROS_WARN("[AIRo PX4] Exiting OFFBOARD mode!");
        }
	}
	return true;
}
...
void AIRO_PX4_FSM::set_ref_with_external_command(){
...
void AIRO_PX4_FSM::external_command_cb(const airo_px4::Reference::ConstPtr& msg){
...
void AIRO_PX4_FSM::takeoff_land_cb(const airo_px4::TakeoffLandTrigger::ConstPtr& msg){
...
bool AIRO_PX4_FSM::external_command_received(const ros::Time& time){
